...
- Makes it so you 'pass props' only once, instead of repeatedly

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
- Actions setup a structure for how state is managed; reducers take the state & action and return a change in state as a result; the store is where everything is stored and prevents 'prop drilling'

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
- Runs us to run side-effects in our action creators, like axios requests

4. What is your favorite state management system you've learned and this sprint? Please explain why!
- Redux, it's basically an advanced version of Context, and Context does so little above regular state management that I'm not sure there's much practical use for it, unless you have 50+ props, in which case you may as well use Redux
